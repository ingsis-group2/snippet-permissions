...
class SnippetController {
    ...
    @PostMapping("")
    fun snippet(@RequestBody snippet: SnippetCreate): ResponseEntity<SnippetDTO> {
        val resp = snippetService.createSnippet(snippet)
        return resp
    }

    @GetMapping("/{userId}")
    fun authToRead(@PathVariable("userId") userId: String): ResponseEntity<List<SnippetLocation>> {
        val resp = snippetService.getAllSnippetsAuthToRead(userId)
        return resp
    }

    @GetMapping("/shared/{userId}")
    fun sharedSnippets(@PathVariable("userId") userId: String): ResponseEntity<List<SnippetLocation>> {
        val resp = snippetService.getSnippetsThatHaveBeenSharedToUser(userId)
        return resp
    }

    @GetMapping("/written/{userId}")
    fun writtenSnippet(@PathVariable("userId") userId: String): ResponseEntity<List<SnippetLocation>> {
        val resp = snippetService.getSnippetsWritten(userId)
        return resp
    }

    @GetMapping("/single")
    fun getSnippet(@RequestParam(value = "snippetId") snippetId: Long, @RequestParam(value = "userId") userId: String): ResponseEntity<SnippetLocation> {
