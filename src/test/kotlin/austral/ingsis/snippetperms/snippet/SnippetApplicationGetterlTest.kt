...
class SnippetApplicationGetterlTest {
    ...
    private fun addReaderIntoSnippet(
        snippetId: Long,
        writerId: String,
        readerId: String,
    ) {
    ...
    private fun getSnippetFormDB(snippetId: Long): Snippet {
        ...
        secondQuery.setParameter("id", snippetId)
        return secondQuery.resultList.firstOrNull() as Snippet
    }
    ...
    fun `should send a list of snippets by reader test`() {
        ...
        val firstId = snippetIds[0]
        val secondId = snippetIds[1]

        this.addReaderIntoSnippet(firstId, "testWriter", "testReader")
        this.addReaderIntoSnippet(secondId, "testWriter", "testReader")
        ...
        val firstSnippet = this.getSnippetFormDB(firstId)
        val secondSnippet = this.getSnippetFormDB(secondId)
        val firstSnippetReaders = firstSnippet.readers
        val secondSnippetReaders = secondSnippet.readers
        assertTrue(firstSnippetReaders.contains("testReader"))
        assertTrue(secondSnippetReaders.contains("testReader"))
    }
    ...
    fun `should send a list of snippets by reader and writer test`() {
        ...
        val newSnippetContent = response.response.contentAsString
        val objectMapper = ObjectMapper()
        val newSnippet = objectMapper.readValue(newSnippetContent, SnippetDTO::class.java)
        this.addReaderIntoSnippet(newSnippet.id, "anotherTestWriter", "testWriter")
